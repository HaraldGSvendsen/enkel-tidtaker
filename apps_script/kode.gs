...
// Automatically triggered on google form submission (magic name, no need to add trigger manually)
...
function getResultsData() {
  ...
  for (let i = 1; i < partData.length; i++) {
    ...
    const hideName = partData[i][5];
    const name = partData[i][5] == "ja" ? "(skjult)" : partData[i][3];
